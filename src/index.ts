 ...
 * 初始化流程：
 * 1. 加载配置 → 创建 Store
 * 2. 创建 FeedChecker → 收集所有工具 → 创建 Router
...
// 创建 Feed 检查器
const feedChecker = new FeedChecker(store);

// 收集所有 AI Tools 并创建路由器
const { definitions, handlers } = collectAllTools({
  store,
  checkFeeds: (installationId, userId) => feedChecker.checkUserFeeds(installationId, userId),
});
